--[[ 
	...
	-Smart Q improvements (avoid tumbling if bad spot like turrets or bundle of enemy heroes, possibly tumble towards condemn spot?)
	-Fix issue with calcDamage function, this appears to have something to do with armor penetration/ghostblade
	...
	-Sort minion array so that minions attacking inhibitors/turrets are first, super minions second and order the so the minion with most aggro is first
	-Fix issue when using selector library, have to reload
	-Record melee object's incoming damage for more accurate last hit
	-Condemn logic, don't condemn enemies is they are low on health as it could just push them away (also need to confirm winduptime and projspeed for condemn)
]]--

if myHero.charName ~= "Vayne" then return end

--[ UPDATE ]--
local local_version = 14526
...
function findTarget(mode, range)
	...
	elseif mode == 1 then --MOST_DANGEROUS
		local MOST_DANGEROUS = 0
		for _, enemy in pairs(enemyHeroes) do
			if GetDistance(myHero, enemy) < range then
...
function autoCombo_OnTick()
	...
	local enemy = Selector and Selector.GetTarget(MOST_DANGEROUS_ADVANCED, 715) or findTarget(1, 715)
	if enemy then
...
function autoHarass_OnTick()
	local Target = Selector and Selector.GetTarget(NEAREST, getTrueRange()) or findTarget(0, getTrueRange())
	if Target then
		lastAttack = getTick()+getLatency()
		if VIP_USER then
